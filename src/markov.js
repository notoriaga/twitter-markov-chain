...
class Markov {
  constructor(tweets) {
    ...
    for (let i = 0; i < hobbit.length; i++) {
      ...
      this.terminals[tokens[tokens.length - 1]] = true;
      this.startwords.push(tokens[0]);
      for (let j = 0; j < tokens.length - 1; j++) {
        if (this.wordstats.hasOwnProperty(tokens[j])) {
          this.wordstats[tokens[j]].push(tokens[j + 1]);
        } else {
          this.wordstats[tokens[j]] = [tokens[j + 1]];
        }
      }
    }
  }
  generate(min) {
    ...
    while (this.wordstats.hasOwnProperty(word)) {
      let next_words = this.wordstats[word];
      word = choice(next_words);
      tweet.push(word);
      if (tweet.length > min && this.terminals.hasOwnProperty(word)) {
        break;
      }
    }
    if (tweet.length < min) {
    ...
    return tweet.reduce((joined, word) => {
      if (word.match(/\s/)) {
        return joined;
      }
      return word.match(/[|;,.?:\-]/) ? `${joined}${word}` : `${joined} ${word}`;
    }, '');
